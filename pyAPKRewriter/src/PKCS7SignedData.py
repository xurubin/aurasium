#!/usr/bin/env python
"""
Originated from http://verify-sigs.googlecode.com/hg/auth_data.py
...
#    spc_info = None
    ...
    def __init__(self, content):
        self.container, rest = decoder.decode(content, asn1Spec=pkcs7.ContentInfo())
        if rest:
            self.trailing_data = rest

        self.signed_data, rest = decoder.decode(self.container['content'], asn1Spec=pkcs7.SignedData())
...
#        self.spc_info, rest = decoder.decode(spc_blob, asn1Spec=spc.SpcIndirectDataContent())
#        if rest: raise Asn1Error('Extra unparsed content.')
...
#                raise Asn1Error('2: Validation of counterSignature failed.')
    def GetJarSigningChain(self):
        signee = self.certificates[self.signing_cert_id]
        chain = [] 
        # Travesal the cert chain
        while True:
            chain.append(signee)
            issuer = signee[0][0]['issuer']
            issuer_dn = str(dn.DistinguishedName.TraverseRdn(issuer[0]))
